...
class Plot:
    ...
    def PlotBombRun(self, result):
        ...
        # Define distance.
        distance = float(get_val(result, "distance", 500, 0))
        radial = float(get_val(result, "radial", 0, 0))

        # Attack parameters.
        attackHdg = float(get_val(result, "attackHdg", 0, 0))
        attackAlt = get_val(result, "attackAlt", "?", 0)
        attackVel = get_val(result, "attackVel", "?", 0)

        # Mission info.
        theatre = get_val(result, "theatre", "Unknown Map")
        missiontime = get_val(result, "clock", "?")
        ...
        # Create title.
        title = str(f'Bombing result of {get_val(result, "player", "Ghostrider")}\n')
        title += str(f'{get_val(result, "rangename", "Unknown Range")}: {get_val(result, "name", "Unknown Target")}\n')
        ...
        # Show bomb circle.
        plt.imshow(self.imageBombCircle, interpolation='none', origin='upper', extent=[-zmax, zmax, -zmax, zmax],
                   clip_on=True)

        # North Up image.
        north = 150
        plt.imshow(self.imageNorthUp, interpolation='none', origin='upper',
                   extent=[north - 30, north + 30, north - 23, north + 37], clip_on=True, zorder=17)
        ...
        # Crater image.
        scrater = 20
        plt.imshow(self.imageCrater, interpolation='none', origin='upper',
                   extent=[x - scrater, x + scrater, y - scrater, y + scrater], clip_on=True)

        # Attack heading arrow in North Up image.
        xa, ya = self._Polar2Cart(30, float(attackHdg))
        ax.arrow(north, north, xa, ya, head_width=5, head_length=5, zorder=501, length_includes_head=True,
                 color="green")
        ...
        offsetbox = TextArea(
            f'{get_val(result, "airframe", "Unknown AC")}\nh={attackAlt} ft\n$v$={attackVel} kts\n$\psi$={attackHdg}°',
        ...
        offsetbox = TextArea(
            f'{get_val(result, "weapon")}\nr={distance} m\n$\phi$={radial}°\n{get_val(result, "quality", "?")}',
        ...
        plt.annotate(timestamp, xy=(0, 0), xycoords='figure fraction', alpha=0.5, color="grey",
                     horizontalalignment='left', verticalalignment="bottom")

        # Real date at right bottom.
        timestamp = datetime.now().strftime(f"%Y-%b-%d (%H:%M:%S)")
        plt.annotate(timestamp, xy=(1, 0.0), xycoords='figure fraction', alpha=0.5, color="grey",
    ...
    def PlotStrafeRun(self, result):
        ...
        actype = get_val(result, "airframe", "Unknown AC")

        roundsFired = get_val(result, "roundsFired", 1)
        roundsHit = get_val(result, "roundsHit", 0)
        invalid = get_val(result, "invalid", False)
        ...
        title += str(f'\n{get_val(result, "rangename", "Unknown Range")}: {get_val(result, "name", "Unknown Target")}')
        ...
        plt.imshow(self.imageStrafePit, interpolation='none', origin='upper', extent=[-zmax, zmax, -zmax, zmax],
                   clip_on=True)
        ...
        for _ in range(1):  # (roundsHit):
            ...
            # Bullet image.
            bullet = 10
            plt.imshow(self.imageBullet, interpolation='none', origin='upper',
                       extent=[x - bullet, x + bullet, y - bullet, y + bullet], clip_on=True)
        ...
        Nfired = get_val(result, "roundsFired", "?")
        Nhits = get_val(result, "roundsHit", "?")
        Accu = Nhits / max(Nfired, 1) * 100
        Qual = get_val(result, "roundsQuality", "?")
        ...
        # Invalid runs.
        if invalid:
            ax.text(0.5, 0.35, '*** INVALID ***', va='bottom', ha='center', transform=ax.transAxes, color='red',
                    fontsize=25, zorder=5000,
    ...
    def PlotTrapSheet(self, result, filename=None):
        ...
        plt.annotate(carrierinfo, xy=(0.99, 0.98), xycoords='figure fraction', alpha=0.6, color="lightsteelblue",
